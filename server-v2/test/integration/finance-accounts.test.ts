...
test('creates a financial account with the registered user', async () => {
    // создаёт финансовый аккаунт вместе с зарегистрированным пользователем
...
test('rolls back registration when financial account creation fails', async () => {
    ...
    try {
        ...
        const result = await database.query('select count(*)::integer as count from users');

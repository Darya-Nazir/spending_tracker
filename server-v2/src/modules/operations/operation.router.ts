...
import {
    operationCreateSchema, operationIdParamsSchema, operationListSchema, operationUpdateSchema,
} from './operation.schemas.ts';
...
export class OperationRouter {
    static create(controller: OperationController, authenticate: Authenticate): Router {
        ...
        router.get(
            '/:id', authenticate.requireAuth, ValidationMiddleware.params(operationIdParamsSchema),
            controller.getById,
        );
        router.post(
        ...
        router.put(
            '/:id', authenticate.requireAuth, ValidationMiddleware.params(operationIdParamsSchema),
            ValidationMiddleware.body(operationUpdateSchema), controller.update,
        );
        router.delete(
            '/:id', authenticate.requireAuth, ValidationMiddleware.params(operationIdParamsSchema),
            controller.delete,
